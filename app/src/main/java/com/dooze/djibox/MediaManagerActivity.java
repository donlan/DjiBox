...
public class MediaManagerActivity extends Activity implements View.OnClickListener {
    ...
    private static final String TAG = MediaManagerActivity.class.getName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initUI();
        App.getAircraftInstance().getCamera().setStorageStateCallBack(new StorageState.Callback() {
            @Override
            public void onUpdate(@NonNull @NotNull StorageState storageState) {
                if (storageState.isInserted()) {
                    ...
                    App.getAircraftInstance().getCamera().setStorageLocation(SettingsDefinitions.StorageLocation.SDCARD, new CommonCallbacks.CompletionCallback() {
                        @Override
                    ...
                    App.getAircraftInstance().getCamera().setStorageLocation(SettingsDefinitions.StorageLocation.INTERNAL_STORAGE, new CommonCallbacks.CompletionCallback() {
                        @Override
                        public void onResult(DJIError djiError) {
                        }
                    });
    ...
    protected void onDestroy() {
        ...
        if (App.getCameraInstance() != null) {
            if (isMavicAir2() || isAir2S() || isM300()) {
                App.getCameraInstance().exitPlayback(djiError -> {
                    if (djiError != null) {
                        App.getCameraInstance().setFlatMode(SettingsDefinitions.FlatCameraMode.PHOTO_SINGLE, djiError1 -> {
                            if (djiError1 != null) {
                ...
                App.getCameraInstance().setMode(SettingsDefinitions.CameraMode.SHOOT_PHOTO, djiError -> {
                    if (djiError != null) {
    ...
    void initUI() {
        ...
        LinearLayoutManager layoutManager = new LinearLayoutManager(MediaManagerActivity.this, RecyclerView.VERTICAL, false);
    ...
    private void setResultToText(final String string) {
        ...
        MediaManagerActivity.this.runOnUiThread(new Runnable() {
            @Override
            public void run() {
        ...
        if (App.getProductInstance() == null) {
            ...
            if (null != App.getCameraInstance() && App.getCameraInstance().isMediaDownloadModeSupported()) {
                ...
                if (null != mMediaManager) {
                    ...
                    if (isMavicAir2() || isAir2S() || isM300()) {
                        App.getCameraInstance().enterPlayback(djiError -> {
                            if (djiError == null) {
                        ...
                        App.getCameraInstance().setMode(SettingsDefinitions.CameraMode.MEDIA_DOWNLOAD, error -> {
                            if (error == null) {
                                DJILog.e(TAG, "Set cameraMode success");
                                showProgressDialog();
                                getFileList();
    ...
    private class FileListAdapter extends RecyclerView.Adapter<ItemHolder> {
        ...
        public ItemHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.media_info_item_new, parent, false);
